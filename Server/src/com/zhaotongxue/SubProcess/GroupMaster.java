...
//    }
    ...
    /*
    public GroupMaster(User user) throws IOException {
        if(user!=null){
...
/*
class GroupProcess implements Runnable {
    User user = null;

    public GroupProcess(User user) {
        this.user = user;
    }
    ...
    public void run() {
        while (true) {
            try {
                String msg = user.recv();
                System.out.println("User " + user.getName() + ":" + msg + "\n");
                boolean exit = false;
                if (msg == "//EXIT") {
                    GroupMaster.getGroupMaster().removeUser(user);
                    msg = user.getName() + " 退出群组聊天";
                    exit = true;
                }
                for (int i = 0; i < GroupMaster.getGroupMaster().getGroupUserNumber(); i++) {
                    if (!GroupMaster.getGroupMaster().getUser(i).equals(user)) {
                        GroupMaster.getGroupMaster().getUser(i).send(msg);
                    }
                }
                if (exit) {
                    GroupMaster.getGroupMaster().removeUser(user);
